...
components (\cref{fig:architecture}): a per-patch encoder, a causal
Transformer predictor, and a distribution-matching regularizer (SIGReg). The
total parameter count is 2.86\,M, of which 1.08\,M live in the encoder and
1.78\,M in the predictor.

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{runs/architecture.png}
  \caption{EEG-LeJEPA architecture. A 4-second EEG window is patchified by
  a Conv1d tokenizer (kernel\,=\,stride\,=\,40 samples; 200\,ms at 200\,Hz)
  into 20 tokens of 192 dimensions, plus a sinusoidal positional embedding.
  A per-patch MLP encoder---deliberately containing no cross-patch
  mixing---produces patch embeddings $z$, finalised with BatchNorm1d (not
  LayerNorm; required to preserve the per-sample magnitude variation that
  SIGReg matches). The same $z$ then feeds two heads: a 4-layer causal
  Transformer predictor that regresses position $t$ onto position $t{+}1$
  (prediction loss), and SIGReg, which matches the flattened embedding
  distribution to an isotropic Gaussian by drawing $M$ random unit vectors
  on $S^{d-1}$ (Cram\'er--Wold) and scoring each one-dimensional projection
  with the Cram\'er--von Mises statistic. A single encoder feeds both
  heads---no EMA, stop-gradient, or teacher--student split.}
...
\begin{equation}
  ...
  \label{eq:cvm}
\end{equation}
where $F$ is the standard normal CDF. CvM is sort-based, numerically well-conditioned, and easily verifiable against \newline \texttt{scipy.stats.cramervonmises}; in our preliminary calibration both statistics produced equivalent optimisation trajectories and we adopt CvM for its implementation simplicity. The complete SIGReg loop is summarised in Algorithm~\ref{alg:sigreg}.
