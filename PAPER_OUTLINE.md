...
> EEG foundation models (LaBraM, EEGPT, NeuroLM) demonstrate that self-supervised pretraining on large EEG corpora produces useful representations, but at parameter scales (5-10M+) and training budgets (thousands of GPU hours) incompatible with on-device deployment. We present **EEG-LeJEPA**, a 2.86M-parameter Joint-Embedding Predictive Architecture for EEG, trained with **Sketched Isotropic Gaussian Regularization** (SIGReg) — a recently-proposed anti-collapse objective that requires neither teacher-student networks, EMA, nor stop-gradients. Pretrained for 10,000 steps on 50 PhysioNet EEGMMIDB subjects in 10 minutes on a single RTX 5090, the model achieves 71.1% accuracy / 0.778 AUC on cross-subject rest-vs-activity classification and 65.7% / 0.711 AUC on left-vs-right motor imagery via linear probe — +9.3 pp / +5.4 pp respectively over a randomly-initialized identical architecture. We demonstrate a clean scaling law across four pretraining scales (17× data range, 100× compute range, monotonic), and an ablation across SIGReg weight λ ∈ {0, 0.1, 0.3, 1.0, 3.0, 10.0} that reveals (i) a sharp optimum at λ=1.0, and (ii) complete representational collapse at λ=0 (probe accuracy regresses to *exactly chance*), empirically validating the SIGReg anti-collapse mechanism. The full training + evaluation pipeline runs on a single GPU in under 15 minutes total at a marginal compute cost under $1.
